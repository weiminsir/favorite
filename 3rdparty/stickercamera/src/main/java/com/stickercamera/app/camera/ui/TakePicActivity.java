...
/**
 * Created by ken on 28/7/15.
 */
public class TakePicActivity extends CameraBaseActivity {
    public final static String PIC_URL = "pic_url";
    public final static String IS_CROP = "is_crop";
    public final static int REQUEST_FOR_GALLERY_PIC = 1;
    ...
    private int photoMargin = Device.dp2px(App.getApp(), 1);
    ...
    private void addPhoto(PhotoItem photoItem) {
        ...
        photo.setOnClickListener(v -> {
            if (v instanceof ImageView && v.getTag() instanceof String) {
                CameraManager.getInst().processPhotoItem(TakePicActivity.this,
    ...
    private void initEvent() {
        ...
        flashBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                TakePicActivity.this.turnLight(cameraInst);
            }
        });
        //前后置摄像头切换
        ...
        //跳转相册
        galaryBtn.setOnClickListener(v -> startActivityForResult(new Intent(TakePicActivity.this, AlbumActivity.class), REQUEST_FOR_GALLERY_PIC));
        //返回按钮
...
//            startActivity(newIntent);
            ...
            intent.putExtra(PIC_URL, getRealFilePath(TakePicActivity.this, result.getData()));
            setResult(RESULT_OK, intent);
            finish();
        } else if (requestCode == REQUEST_FOR_GALLERY_PIC && resultCode == RESULT_OK) {
            Intent intent = new Intent();
            intent.putExtra(PIC_URL, result.getStringExtra(AlbumActivity.PIC_URL));
            setResult(RESULT_OK, intent);
            finish();
        }
...
//            Intent intent = new Intent();
//            intent.putExtra(PIC_URL, getRealFilePath(TakePicActivity.this, uri));
//            setResult(RESULT_OK, intent);
//            finish();
//        }
    }
    ...
    private void showPoint(int x, int y) {
        if (parameters.getMaxNumMeteringAreas() > 0) {
            ...
            //xy变换了
            int rectY = -x * 2000 / App.getScreenWidth() + 1000;
            int rectX = y * 2000 / App.getScreenHeight() - 1000;
        ...
        protected void onPostExecute(String result) {
            ...
            if (StringUtils.isNotEmpty(result)) {
                ...
                if (mIsCrop) {
                    ...
                    Intent intent = new Intent(TakePicActivity.this, CropPhotoActivity.class);
                    intent.putExtra("cover_scale",scaleSize);
                    intent.setData(uri);
                    startActivityForResult(intent, Crop.REQUEST_CROP);
                } else {
                    Intent intent = new Intent();
                    intent.putExtra(PIC_URL, result);
                    setResult(RESULT_OK, intent);
                    finish();
                }
//                CameraManager.getInst().processPhotoItem(TakePicActivity.this,
//                        new PhotoItem(result, System.currentTimeMillis()));
    ...
    private Camera.Size findBestPreviewResolution() {
        ...
        // 移除不符合条件的分辨率
        double screenAspectRatio = (double) App.getScreenWidth()
                / (double) App.getScreenHeight();
        ...
        while (it.hasNext()) {
            ...
            if (maybeFlippedWidth == App.getScreenWidth()
                    && maybeFlippedHeight == App.getScreenHeight()) {
